...
@section('content')
    ...
    <main class="mx-auto max-w-[1200px] space-y-8 px-4 py-6">
        <section class="relative" data-banner-carousel>
            <div class="absolute right-0 top-0 z-10 flex h-10 w-[90px] items-center justify-between rounded-full border border-[#e5e9f1] bg-page p-1">
            ...
            <div class="overflow-hidden rounded-2xl bg-black">
                @foreach ([1, 2, 3] as $slide)
                    <img
                        data-banner-slide
                        src="{{ asset('images/home/banner.svg') }}"
                        alt=""
                        @class(['h-[263px] w-full object-cover', 'hidden' => $slide !== 1])
                        width="1200"
                        height="263"
                    >
                @endforeach
            ...
            <div class="absolute bottom-4 left-1/2 flex -translate-x-1/2 gap-1">
                @foreach ([0, 1, 2] as $index)
                    <button type="button" data-banner-dot @class(['h-1 w-5 rounded-full', 'bg-white' => $index === 0, 'bg-white/45' => $index !== 0])></button>
                @endforeach
            </div>
        </section>

        <section class="space-y-4 rounded-2xl bg-white p-5 shadow-[0px_10px_17px_rgba(20,40,80,0.08)]">
            <div class="flex gap-4 overflow-x-auto pb-1">
                ...
                @foreach ($services as $service)
                    <button type="button" data-service-item class="flex w-[76px] shrink-0 flex-col items-center gap-2 transition duration-200">
                        <span @class([
                            'flex size-[72px] items-center justify-center overflow-hidden rounded-2xl border-2 p-0.5 shadow-[0px_4px_8px_#d1d9e4] transition duration-200',
                            'bg-black' => $service['active'] ?? false,
                        ]) style="border-color: {{ $service['border'] }}">
                ...
                <button type="button" data-service-item class="flex w-[76px] shrink-0 flex-col items-center gap-2 transition duration-200">
                    <span class="flex size-[72px] items-center justify-center rounded-2xl border-2 border-[#dfe5ef] bg-[#f2f4f7]">
            ...
            <div class="grid gap-4 xl:grid-cols-[minmax(0,1.1fr)_minmax(0,1fr)_minmax(0,1.2fr)_200px] xl:items-center">
                <div class="flex items-center gap-3">
                    <img src="{{ asset('images/home/steam.png') }}" alt="Steam" class="size-[72px] rounded-2xl border-2 border-[#1482b3] object-cover shadow-[0px_4px_8px_#d1d9e4]" width="72" height="72">
                    <div>
                        <div class="flex items-center gap-2">
                            <p class="text-base font-bold tracking-tight text-text">Пополнение Steam</p>
                ...
                <label class="flex h-16 max-w-[300px] items-center justify-between rounded-xl bg-page px-5">
                    <span class="flex min-w-0 flex-1 items-center gap-3">
                        ...
                        <input type="text" placeholder="Логин Steam" class="w-full bg-transparent text-[15px] font-bold text-muted outline-none placeholder:text-muted">
                    </span>
                ...
                <div class="flex h-16 max-w-[380px] items-center justify-between rounded-xl bg-page px-4">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-lg font-bold text-text">500₽</p>
                        </div>
                    </div>
                ...
                <button type="button" class="h-16 w-full rounded-xl bg-black text-base font-bold text-white transition hover:bg-[#222] xl:w-[200px]">
                    Оплатить 500$
                </button>
            </div>
        </section>
